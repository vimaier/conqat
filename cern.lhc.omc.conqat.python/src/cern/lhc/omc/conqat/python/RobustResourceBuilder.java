...
import org.conqat.engine.resource.build.Container;
...
@AConQATProcessor(description = "Processor for constructing resources from content accessors. Catches " +
		"AssertionExceptions during inserting of nodes.")
/**
 * This is a complete copy of {@link ResourceBuilder}. Needed since this class makes problems with symbolic links.
 * This class catches assertion exceptions in the static function insert.
 * @author vimaier
 *
 */
public class RobustResourceBuilder extends ConQATProcessorBase {
		...
		for (int i = start; i < segments.length - 1; ++i) {
			...
			if (!(child instanceof ContainerBase<?>)) {
				...
				try{
					...
					// We cannot acces the logger from the static function...
					// We use sysout instead of syserr because we assume this exception.
