 ...
 * @version $Rev: 41751 $
 * @ConQAT.Rating GREEN Hash: 0C578C1E00C148E02DADB1846413B219
 * 
 * @param <N1>
 *            The first node type
 * @param <N2>
...
public class MaxWeightMatching<N1, N2> {
	 ...
	 */
	private final Map<Long, Double> weightCache = new HashMap<Long, Double>();
	...
	private double getWeight(int i1, int i2) {
		...
		// we use a long as a key as it allows for a compact representation of
		// both ints.
		Long key = ((long) i1 << 32) + i2;

		Double result = weightCache.get(key);
		if (result == null) {
			if (swapped) {
				result = weightProvider.getConnectionWeight(nodes1.get(i2),
						nodes2.get(i1));
			} else {
				result = weightProvider.getConnectionWeight(nodes1.get(i1),
						nodes2.get(i2));
			}
			weightCache.put(key, result);
		}
		return result;
	}
